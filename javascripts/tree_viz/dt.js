...
var dt = (function () {
    ...
    function DecisionTree(builder) {        
        this.root = buildDecisionTree({
            ...
            entropyThrehold: builder.entropyThrehold || 0.0,
            maxTreeDepth: builder.maxTreeDepth || Infinity
        });
    }
    ...
    function buildDecisionTree(builder) {
        ...
        for (var i = trainingSet.length - 1; i >= 0; i--) {
            ...
            for (var attr in item) {
                ...
                if (currGain > bestSplit.gain) {
                    ...
                    if ((currGain >= bestSplit.gain) && (currSplit.match.length > 0) && (currSplit.notMatch.length > 0)) {
                        // remember pairs 'attribute-predicate-value'
                        // which provides informational gain
                        bestSplit = currSplit;
                        bestSplit.predicateName = predicateName;
        ...
        var result = trainingSet.map(x => x[categoryAttr])
        // can't find optimal split
        cat_counts = Counter(result);
        top_category = Object.keys(cat_counts).reduce(function(a, b){ return cat_counts[a] > cat_counts[b] ? a : b });
